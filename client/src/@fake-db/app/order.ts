...
// ** Types
import { OrderType } from 'src/types/app/orderTypes'
  ...
  orders: [
    ...
    },
    {
      ...
      to: [
        {
          name: 'me',
          order: 'johndoe@sciencedaily.com'
        }
      ],
    ...
    },
    {
      id: 9,
      from: {
        order: 'hfrostdyke4@scientificamerican.com',
    ...
    },
    {
      ...
      to: [
        {
          name: 'me',
          order: 'johndoe@odnoklassniki.ru'
        }
      ],
    ...
    },
    {
      id: 17,
      from: {
        order: 'nbartlesc@merriam-webster.com',
    ...
    },
    {
      id: 24,
      from: {
        order: 'douldcottj@yellowpages.com',
    ...
    },
    {
      id: 28,
      from: {
        order: 'mosgarby1@accuweather.com',
        name: 'Milena Osgarby',
        avatar: '/images/avatars/3.png'
      },
      to: [
    ...
    },
    {
      id: 30,
      from: {
        order: 'gabramow2@elegantthemes.com',
...
let paramsFilteredMails: OrderType[] = []

// ------------------------------------------------
// GET: Return Emails
mock.onGet('/app/order/allOrders').reply(() => {
  return [200, { orders: data.orders }]
})

// ------------------------------------------------
// GET: Return Emails
mock.onGet('/app/order/orders').reply(config => {
  const { q = '', folder = 'inbox', label } = config.params

  const queryLowered = q.toLowerCase()

  function isInFolder(email: OrderType) {
  ...
  // ------------------------------------------------
  const ordersMeta = {
    inbox: data.orders.filter((email: OrderType) => !email.isRead && email.folder === 'inbox').length,
    draft: data.orders.filter((email: OrderType) => email.folder === 'draft').length,
    spam: data.orders.filter((email: OrderType) => !email.isRead && email.folder === 'spam').length
  }
...
// ------------------------------------------------
mock.onPost('/app/order/update-orders-label').reply(config => {
  const { orderIds, label } = JSON.parse(config.data).data

  function updateMailLabels(email: OrderType) {
  ...
  data.orders.forEach((email: OrderType) => {
    if (orderIds.includes(email.id)) updateMailLabels(email)
  })
...
// ------------------------------------------------
mock.onGet('/app/order/get-order').reply(config => {
  const { id } = config.params

  const orderId = Number(id)

  const order = paramsFilteredMails.find((i: OrderType) => i.id === orderId)
  if (order) {
    const mailIndex = paramsFilteredMails.findIndex((i: OrderType) => i.id === order.id)
    mailIndex > 0 ? (order.hasPreviousOrder = true) : (order.hasPreviousOrder = false)
    mailIndex < paramsFilteredMails.length - 1 ? (order.hasNextOrder = true) : (order.hasNextOrder = false)
  }

  return order ? [200, order] : [404]
})

// ------------------------------------------------
// POST: Update Email
// ------------------------------------------------
mock.onPost('/app/order/update-order').reply(config => {
  const { orderIds, dataToUpdate } = JSON.parse(config.data).data

  function updateMailData(email: OrderType) {
    Object.assign(email, dataToUpdate)
  }

  data.orders.forEach((email: OrderType) => {
    if (orderIds.includes(email.id)) updateMailData(email)
  })
...
// ------------------------------------------------
mock.onGet('/app/order/paginate-order').reply(config => {
  const { dir, orderId } = config.params
  const currentEmailIndex = paramsFilteredMails.findIndex((e: OrderType) => e.id === orderId)
  const newEmailIndex = dir === 'previous' ? currentEmailIndex - 1 : currentEmailIndex + 1

  const newEmail = paramsFilteredMails[newEmailIndex]

  if (newEmail) {
    const mailIndex = paramsFilteredMails.findIndex((i: OrderType) => i.id === newEmail.id)
    mailIndex > 0 ? (newEmail.hasPreviousOrder = true) : (newEmail.hasPreviousOrder = false)
    mailIndex < paramsFilteredMails.length - 1 ? (newEmail.hasNextOrder = true) : (newEmail.hasNextOrder = false)
  }
